...
//{
//	NSArray *player=[NSArray arrayWithObjects:[GKLocalPlayer localPlayer].playerID,nil];
//	GKLeaderboard *query =[[ [GKLeaderboard alloc] initWithPlayerIDs:player] autorelease]; 
//	if (query != nil) {
...
//				// handle the error.
//			}else {
//				[delegate onRetrieveLocalPlayerScore:scores];
//			}
//            
//			if (scores != nil) {
//				// process the score information.
//                //	[delegate onRetrieveLocalPlayerScore:scores];
//				//localPlayerScore=[scores objectAtIndex:0];
//			}
//            
//		}];
//	}
...
-(void) findMatchForRequest:(GKMatchRequest*)request
{
	...
	[[GKMatchmaker sharedMatchmaker] findMatchForRequest:request withCompletionHandler:^(GKMatch* match, NSError* error)
	{
		...
		if (match != nil)
		{
//            CCLOG(@"%@",[match.playerIDs objectAtIndex:0]);
...
-(void) showLeaderboardWithCategory:(NSString*)category
{
	...
	if (leaderboardVC != nil)
	{
        ...
        [leaderboardVC.navigationController popViewControllerAnimated:NO ];
	}
}
...
//	[self dismissModalViewController];
	...
	//NSLog(@"Match started! Expected Player Count:%d  %@",currentMatch.expectedPlayerCount, currentMatch.playerIDs);
